...
const DEFAULT_GLOBAL = { injectIntoPrompt: true, characters: {}, activeChar: null };

function getGlobal() {
    if (!extension_settings[EXT_NAME]) extension_settings[EXT_NAME] = Object.assign({}, DEFAULT_GLOBAL);
    return extension_settings[EXT_NAME];
}

function getCurrentCharName() {
    try { const ctx = getContext(); return ctx?.characters?.[ctx?.characterId]?.name || '__global__'; }
...
function trimLabel(t) { return ['', 'I триместр', 'II триместр', 'III триместр'][t] || ''; }
...
function showToast(msg, type = 'warn') {
    ...
    toast.style.cssText = `position:fixed;bottom:24px;right:24px;z-index:99999;background:${type==='danger'?'rgba(180,60,60,0.92)':'rgba(160,100,110,0.92)'};color:#fff;padding:10px 16px;border-radius:10px;font-family:Georgia,serif;font-size:13px;box-shadow:0 4px 20px rgba(0,0,0,0.4);max-width:280px;line-height:1.4;`;
...
function updateBadge() {
    ...
    const active = (cd.threats || []).filter(t => !t.resolved && DANGER_THREATS.includes(t.type));
    if (active.length > 0) { _badgeEl.style.display = 'flex'; _badgeEl.textContent = active.length; }
    else _badgeEl.style.display = 'none';
}

function checkDangerAndNotify(type) {
    if (DANGER_THREATS.includes(type)) { showToast(`⚠️ Опасный симптом: ${type}`, 'danger'); updateBadge(); }
}
...
function buildPromptInjection() {
    ...
    if (cd.mode === 'cycle') {
        ...
        if (cyc) {
            ...
            if ((cd.conceptionAttempts||[]).length) lines.push(`Попыток зачатия: ${cd.conceptionAttempts.length}`);
        }
    ...
    lines.push(`Настроение: ${cd.customMood || cd.mood}, самочувствие: ${cd.wellbeing}`);
    const active = (cd.threats || []).filter(t => !t.resolved);
    if (active.length) { lines.push('Активные угрозы:'); active.forEach(t => lines.push(`  - [${t.type}] ${t.description} (с ${fmtDate(t.date)})`)); }
...
#pt-panel{font-family:'Georgia',serif;padding:12px;color:#4a3f3f;font-size:13px;line-height:1.6;max-height:82vh;overflow-y:auto;scrollbar-width:thin;scrollbar-color:#d4a0a0 transparent}
#pt-panel::-webkit-scrollbar{width:3px}
#pt-panel::-webkit-scrollbar-thumb{background:rgba(200,140,160,0.3);border-radius:10px}
.pt-char-bar{display:flex;align-items:center;gap:6px;margin-bottom:10px;padding:7px 10px;background:rgba(255,220,230,0.12);border:1px solid rgba(210,150,170,0.2);border-radius:10px;font-size:11px}
.pt-char-name{font-weight:600;color:#d4a0b0;flex:1}
.pt-char-dot{width:7px;height:7px;border-radius:50%;background:#c98fa0;animation:ptPulse 2s infinite}
.pt-card{background:rgba(255,230,235,0.07);border:1px solid rgba(210,150,170,0.18);border-radius:12px;padding:11px 13px;margin-bottom:9px}
.pt-card-title{font-size:10px;text-transform:uppercase;letter-spacing:0.1em;color:rgba(210,150,170,0.55);margin-bottom:8px}
.pt-tabs{display:flex;gap:4px;margin-bottom:10px}
.pt-tab{flex:1;padding:6px 4px;border-radius:10px;border:1px solid rgba(210,150,170,0.2);background:rgba(200,130,150,0.08);color:rgba(200,160,175,0.55);font-size:10px;font-family:inherit;cursor:pointer;text-align:center;transition:all 0.2s}
.pt-tab.active{background:rgba(200,130,150,0.25);border-color:rgba(210,150,170,0.4);color:#e0b0c0}
.pt-stat{display:flex;justify-content:space-between;align-items:center;padding:4px 0;border-bottom:1px solid rgba(200,150,170,0.08);font-size:12px}
.pt-stat:last-child{border-bottom:none}
.pt-stat-label{color:rgba(200,160,175,0.55)}
.pt-stat-value{color:#d4b0bc;font-weight:500}
.pt-badge{display:inline-block;padding:2px 9px;border-radius:20px;font-size:10px;font-weight:600;background:rgba(200,150,170,0.18);color:#d4a0b8;border:1px solid rgba(200,140,165,0.25)}
.pt-badge.t1{background:rgba(180,220,200,0.15);color:#90c0a0}
.pt-badge.t2{background:rgba(220,200,150,0.15);color:#c8b070}
.pt-badge.t3{background:rgba(220,160,180,0.18);color:#d4a0b0}
.pt-badge.fertile{background:rgba(160,220,180,0.2);color:#80c090;animation:ptPulse 1.5s infinite}
.pt-badge.ovulation{background:rgba(220,190,100,0.2);color:#c8a840;animation:ptPulse 1s infinite}
...
.pt-input,.pt-select,.pt-textarea{width:100%;background:rgba(255,225,230,0.07);border:1px solid rgba(210,150,170,0.2);border-radius:8px;padding:5px 9px;color:rgba(220,180,195,0.85);font-size:11px;font-family:inherit;outline:none;box-sizing:border-box;margin-top:3px;transition:border-color 0.2s}
.pt-input:focus,.pt-select:focus,.pt-textarea:focus{border-color:rgba(200,130,150,0.45)}
.pt-textarea{resize:vertical;min-height:48px}
.pt-row{display:flex;align-items:center;gap:8px;margin:5px 0}
.pt-row label{font-size:11px;color:rgba(200,155,170,0.5);min-width:80px;flex-shrink:0}
.pt-btn{display:inline-block;padding:5px 12px;border-radius:20px;border:1px solid rgba(210,140,160,0.3);background:rgba(200,120,145,0.12);color:rgba(220,160,180,0.8);font-size:11px;font-family:inherit;cursor:pointer;transition:all 0.2s}
...
.pt-toggle{display:flex;align-items:center;gap:8px;font-size:12px;color:rgba(210,170,185,0.7);cursor:pointer;margin-bottom:5px}
.pt-toggle input[type=checkbox]{accent-color:#c98fa0;width:14px;height:14px;cursor:pointer}
.pt-threat-item{display:flex;align-items:flex-start;gap:8px;padding:7px 9px;background:rgba(255,160,160,0.05);border-radius:8px;margin:4px 0;border-left:2px solid rgba(220,120,120,0.35);font-size:11px}
...
.pt-attempt-item{display:flex;justify-content:space-between;align-items:center;padding:4px 8px;background:rgba(200,180,220,0.07);border-radius:6px;margin:3px 0;font-size:11px;color:rgba(200,170,210,0.7)}
.pt-cycle-day{display:flex;align-items:center;gap:6px;padding:5px 8px;border-radius:8px;font-size:11px;color:rgba(200,165,180,0.75);background:rgba(200,150,170,0.06);margin-bottom:4px}
.pt-cycle-day-num{font-weight:700;color:#d4a0b8;min-width:24px}
.pt-history-item{padding:4px 0;border-bottom:1px solid rgba(200,140,160,0.08);font-size:10px;color:rgba(200,155,170,0.5)}
...
.pt-footer{font-size:10px;color:rgba(180,130,150,0.3);text-align:center;margin-top:4px}
#pt-danger-badge{position:absolute;top:-4px;right:-4px;width:16px;height:16px;background:#c03040;border-radius:50%;font-size:9px;color:white;display:none;align-items:center;justify-content:center;font-weight:700;z-index:10;animation:ptPulse 1.2s infinite}
`;
function buildHTML() {
    ...
    <div class="pt-card">
        <label class="pt-toggle"><input type="checkbox" id="pt-inject"><span>Инъекция в промпт</span></label>
    </div>
    <div class="pt-tabs">
    ...
    <div class="pt-section" id="pt-section-cycle">
        <div class="pt-card">
            ...
            <div class="pt-row"><label>Начало цикла</label><input type="date" id="pt-cycle-start" class="pt-input"></div>
            <div class="pt-row"><label>Длина цикла</label><input type="number" id="pt-cycle-length" class="pt-input" min="21" max="45" value="28" style="width:70px;"><span style="font-size:11px;color:rgba(200,155,170,0.5);">дней</span></div>
            <div class="pt-row"><label>День овуляции</label><input type="number" id="pt-ovulation-day" class="pt-input" min="1" max="40" value="14" style="width:70px;"><span style="font-size:11px;color:rgba(200,155,170,0.5);">от начала</span></div>
            <hr class="pt-divider"><div id="pt-cycle-stats"></div>
        </div>
        <div class="pt-card">
            <div class="pt-card-title">Попытки зачатия</div>
            <div id="pt-attempts-list"></div>
            <hr class="pt-divider">
            <div class="pt-row"><label>Дата</label><input type="date" id="pt-attempt-date" class="pt-input"></div>
            <div class="pt-row"><label>Заметка</label><input type="text" id="pt-attempt-note" class="pt-input" placeholder="необязательно"></div>
        ...
        <div class="pt-card">
            ...
            <div class="pt-row"><label>День цикла</label><input type="number" id="pt-sym-day" class="pt-input" min="1" max="45" placeholder="1–45" style="width:70px;"></div>
            <div class="pt-row"><label>Симптом</label><input type="text" id="pt-sym-text" class="pt-input" placeholder="описание..."></div>
    ...
    <div class="pt-section" id="pt-section-pregnancy">
        <div class="pt-card">
            ...
            <div class="pt-row"><label>Дата зачатия</label><input type="date" id="pt-conception" class="pt-input"></div>
            <hr class="pt-divider"><div id="pt-pregnancy-stats"></div>
        </div>
        <div class="pt-card">
            <div class="pt-card-title">Состояние персонажа</div>
            <div class="pt-row"><label>Настроение</label>
                <select id="pt-mood" class="pt-select"><option>нейтральное</option><option>радостное</option><option>тревожное</option><option>подавленное</option><option>раздражённое</option><option>умиротворённое</option><option>другое</option></select>
            </div>
            <div class="pt-row" id="pt-custom-mood-row" style="display:none;"><label>Уточнить</label><input type="text" id="pt-custom-mood" class="pt-input" placeholder="своё настроение"></div>
            <div class="pt-row"><label>Самочувствие</label>
                <select id="pt-wellbeing" class="pt-select"><option>хорошее</option><option>удовлетворительное</option><option>плохое</option><option>токсикоз</option><option>постельный режим</option><option>критическое</option></select>
            </div>
        </div>
        <div class="pt-card">
            ...
            <hr class="pt-divider">
            <div class="pt-row"><label>Тип</label>
                <select id="pt-threat-type" class="pt-select"><option>угроза выкидыша</option><option>токсикоз</option><option>кровотечение</option><option>высокое давление</option><option>преэклампсия</option><option>боли</option><option>инфекция</option><option>слабость</option><option>стресс</option><option>замершая беременность</option><option>другое</option></select>
            </div>
            <div class="pt-row"><label>Описание</label><input type="text" id="pt-threat-desc" class="pt-input" placeholder="подробности..."></div>
    ...
    <div class="pt-section" id="pt-section-postpartum">
        <div class="pt-card">
            ...
            <div class="pt-row"><label>Пол</label><select id="pt-baby-gender" class="pt-select"><option>неизвестен</option><option>мальчик</option><option>девочка</option></select></div>
            <hr class="pt-divider"><div id="pt-baby-stats"></div><hr class="pt-divider">
            <div class="pt-row"><label>Здоровье</label><select id="pt-baby-health" class="pt-select"><option>хорошее</option><option>колики</option><option>болеет</option><option>критическое</option></select></div>
            <div class="pt-row"><label>Настроение</label><select id="pt-baby-mood" class="pt-select"><option>спокойное</option><option>плачет</option><option>игривое</option><option>сонное</option></select></div>
            <div style="margin-top:6px;"><div class="pt-card-title">Заметки</div><textarea id="pt-baby-notes" class="pt-textarea" placeholder="заметки о ребёнке..."></textarea></div>
        </div>
        <div class="pt-card">
            <div class="pt-card-title">Состояние матери</div>
            <div class="pt-row"><label>Настроение</label><select id="pt-mood-post" class="pt-select"><option>нейтральное</option><option>радостное</option><option>тревожное</option><option>послеродовая депрессия</option><option>подавленное</option><option>умиротворённое</option></select></div>
            <div class="pt-row"><label>Самочувствие</label><select id="pt-wellbeing-post" class="pt-select"><option>хорошее</option><option>восстанавливается</option><option>плохое</option><option>осложнения</option></select></div>
        </div>
        <div class="pt-card">
            <div class="pt-card-title">Угрозы (послеродовые)</div>
            <div id="pt-threats-list-post"></div>
            <hr class="pt-divider">
            <div class="pt-row"><label>Тип</label><select id="pt-threat-type-post" class="pt-select"><option>кровотечение</option><option>инфекция</option><option>лихорадка</option><option>мастит</option><option>депрессия</option><option>слабость</option><option>другое</option></select></div>
            <div class="pt-row"><label>Описание</label><input type="text" id="pt-threat-desc-post" class="pt-input" placeholder="подробности..."></div>
...
function renderThreats(isPost = false) {
    const cd = getCharData();
    const el = document.getElementById(isPost ? 'pt-threats-list-post' : 'pt-threats-list');
    if (!el) return;
...
function renderHistory() {
    ...
    el.innerHTML = hist.slice(0,15).map(h => `<div class="pt-history-item">${fmtDate(h.date)} · ${h.field}: ${h.oldVal} → ${h.newVal}</div>`).join('');
}
function renderAll() {
    const cd = getCharData();
    const nameEl = document.getElementById('pt-char-name');
    if (nameEl) nameEl.textContent = getCurrentCharName();
    ...
    renderCycleStats(); renderAttempts(); renderSymptoms();
    renderPregnancyStats(); renderBabyStats();
    renderThreats(false); renderThreats(true); updateBadge();
}

function bindEvents() {
    document.getElementById('pt-inject')?.addEventListener('change', e => {
        getGlobal().injectIntoPrompt = e.target.checked; saveSettingsDebounced();
    });
    document.querySelectorAll('.pt-tab').forEach(btn => {
        btn.addEventListener('click', () => {
            getCharData().mode = btn.dataset.tab;
            saveSettingsDebounced(); renderAll(); checkFertileWindow();
        });
    });
    const bindCycle = (id, key, parse) => {
        document.getElementById(id)?.addEventListener('change', e => {
            const cd = getCharData();
            cd[key] = parse ? parse(e.target.value) : e.target.value;
            saveSettingsDebounced(); renderCycleStats();
        });
    };
    ...
    document.getElementById('pt-add-sym')?.addEventListener('click', () => {
        ...
        saveSettingsDebounced(); renderSymptoms();
    });
    ...
    bindField('pt-mood','mood'); bindField('pt-wellbeing','wellbeing');
    bindField('pt-mood-post','mood'); bindField('pt-wellbeing-post','wellbeing');
    bindField('pt-baby-health','babyHealth'); bindField('pt-baby-mood','babyMood');
    bindField('pt-baby-gender','babyGender');
    document.getElementById('pt-custom-mood')?.addEventListener('input', e => { getCharData().customMood = e.target.value; saveSettingsDebounced(); });
    ...
    document.getElementById('pt-baby-name')?.addEventListener('input', e => { getCharData().babyName = e.target.value; saveSettingsDebounced(); });
    document.getElementById('pt-baby-notes')?.addEventListener('input', e => { getCharData().babyNotes = e.target.value; saveSettingsDebounced(); });
    const addThreat = (typeId, descId, isPost) => {
        ...
        document.getElementById(btnId)?.addEventListener('click', () => {
            ...
            checkDangerAndNotify(type); saveSettingsDebounced();
